...
bool Rhombus::Draw(double x, double y)
{
...
{
	double e,f;
	e = sqrt(pow((this->A.GetX() - this->C.GetX()), 2) + pow((this->A.GetY() - this->C.GetY()), 2));
	f = sqrt(pow((this->B.GetX() - this->D.GetX()), 2) + pow((this->B.GetY() - this->D.GetY()), 2));
	return e*f/2;
};
void Rhombus::GetType()
{
	cout << "Rhombus" << endl;
};